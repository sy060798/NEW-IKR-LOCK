    ...
    wb.SheetNames.forEach(s => {
      ...
      json.forEach(r => {
        ...
        dataIKR.forEach(row => {
          ...
          let sudah = row.approvedList.some(x =>
            String(x.wo).trim() === wo
          );
...
function sortData() {
  ...
  dataIKR.sort((a,b)=>{
    ...
    if((bulanMap[a.bulan]||0)!==(bulanMap[b.bulan]||0))
      return (bulanMap[a.bulan]||0)-(bulanMap[b.bulan]||0);
...
<td>
<span onclick="showDetail(${i})" style="color:cyan;cursor:pointer">
${d.jumlah}
</span>
</td>
...
function showLoading(text="Loading..."){
  ...
  box.innerHTML=`<div style="color:#fff">${text}</div>`;
}
